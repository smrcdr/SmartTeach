...
This repository is an npm workspace monorepo. Frontend code lives in `apps/web`, backend code in `apps/api`, and shared package code in `packages/shared/src`. Vue pages are under `apps/web/src/pages`, reusable UI under `apps/web/src/components`, mock data under `apps/web/src/data`, and global styles in `apps/web/src/styles/base.css`. NestJS source lives in `apps/api/src`, organized by feature modules with `controller`, `service`, `mapper`, `dto`, and schema files. Prisma schema, migrations, and seed logic are in `apps/api/prisma`. Product and architecture notes live in `docs`. Generated output in `dist` should not be edited manually.
...
Use TypeScript throughout. Follow the existing style: 2-space indentation, single quotes, and no trailing semicolons. Vue components use `<script setup lang="ts">`; page components use a `Page` suffix such as `ProfilePage.vue`, while reusable components use PascalCase such as `CourseCard.vue`. In the API, keep NestJS feature folders cohesive and match existing names like `groups.service.ts` and `create-group-request.dto.ts`.

## Testing Guidelines
API tests live in `apps/api/src/tests` and use `.test.ts` or `.e2e.test.ts` suffixes. Prefer adding tests close to the affected backend behavior. For frontend work, there is no automated suite yet, so run `npm run build` and manually verify affected routes in the browser.
