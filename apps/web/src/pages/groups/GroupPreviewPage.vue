...
const accessPanelLead = computed(() => {
  ...
  if (group.value.accessMode === 'OPEN') {
    return 'После вступления вы сразу попадёте во внутреннее рабочее пространство группы и увидите доступные модули.'
  }
  ...
  return 'Текущий сервер не открывает отдельный сценарий просмотра для закрытых групп.'
})
