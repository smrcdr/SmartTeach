...
<template>
  ...
  <div v-else-if="event" class="page-shell">
    <header class="page-header">
      <span class="page-eyebrow">Рабочее пространство / Расписание / Редактирование</span>
      <h1 class="page-title">Редактирование пользовательского события вынесено на отдельную страницу.</h1>
      <p class="page-lead">
        Здесь можно менять временной слот, описание, локацию и статус записи. Если событие перевести в `Отменено`, оно
        исчезнет из ленты, но останется доступным по этому маршруту.
      </p>
    </header>
    ...
    <div v-if="event.status === 'CANCELLED'" class="panel-note">
      Событие находится в статусе `Отменено`, поэтому в основной ленте его больше не видно. Здесь запись всё
    ...
    <div class="section-grid">
      ...
      <AppCard tone="accent" class="span-4 schedule-event-danger">
        ...
        <ul class="list-copy">
          <li>Для временного скрытия из ленты безопаснее оставить запись и сменить статус на `Отменено`.</li>
          <li>Удаление нужно только когда слот создан ошибочно и не должен оставаться в истории рабочего пространства.</li>
        </ul>
      </AppCard>
    </div>
  </div>
