  ...
  <div class="section-grid">
    ...
    <AppCard tone="accent" class="span-4 submission-form__card submission-form__actions-card">
      <div class="submission-form__header">
        <div>
          ...
          <p class="muted">
            После `Отправить` попытка становится доступной только для чтения. Новая попытка создаётся уже из деталей задания.
          </p>
        </div>
      </div>
    ...
    <AppCard class="span-8 submission-form__card">
      ...
      <p v-if="keptFiles.length === 0 && newFiles.length === 0" class="muted">
        Во вложениях пока пусто. Это допустимо: попытка может состоять только из текста.
      </p>
    </AppCard>
    ...
    <AppCard class="span-4 submission-form__card">
      ...
      <ul class="list-copy">
        ...
        <li>После отправки предыдущая попытка не теряется и остаётся в деталях задания.</li>
        <li>Проверка и комментарии происходят уже после перехода из состояния отправки в состояние проверки.</li>
      </ul>
    </AppCard>
  </div>
</template>
