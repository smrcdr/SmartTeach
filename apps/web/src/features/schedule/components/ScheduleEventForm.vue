...
const sectionLead = computed(() =>
  props.mode === 'create'
    ? 'Событие создаётся на отдельной странице: здесь нет декоративного календаря, только явные поля и рабочий контекст.'
    : 'Редактирование оставляет полный контроль над названием, временным слотом, локацией и статусом записи.',
)
const statusHint = computed(() =>
  status.value === 'CANCELLED'
    ? 'Отменённые события не попадают в ленту группы, но остаются доступны на странице редактирования.'
    : 'Статус управляется явно: запланированное событие остаётся видимым в общем расписании.',
)
...
<template>
  <div class="section-grid">
    ...
    <AppCard tone="accent" class="span-4 schedule-event-form__card">
      ...
      <ul class="list-copy">
        <li>Лента расписания смешивает пользовательские события с уроками и дедлайнами в одном дневном потоке.</li>
        <li>Архивная группа остаётся только для чтения, поэтому форма показывается только для ролей с правом изменения.</li>
        <li>Если сохранить статус `Отменено`, событие останется доступным на странице редактирования, но исчезнет из ленты.</li>
      </ul>
    </AppCard>
  </div>
</template>
